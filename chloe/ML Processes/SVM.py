...
def plot_5component_pca_scatter(X_features, y_labels, train_mask, eval_mask, test_acc, test_f1, pc_x=1, pc_y=2, title="Lower-Star PCA Dimensionality Reduction"):
    ...
    metrics_text = f"SVM Evaluation Accuracy: {test_acc * 100:.2f}%\nSVM Evaluation F1 Score: {test_f1 * 100:.2f}%"
    plt.text(
...
if __name__ == '__main__':
    ...
    if not all_raw_images:
        ...
        X_train, y_train = process_directory_subset(train_images, "TRAINING")
        X_eval, y_eval = process_directory_subset(eval_images, "EVALUATION")
        ...
        # Step 3: Run Support Vector Machine Evaluation
        if len(X_train) > 0 and len(X_eval) > 0:
            try:
                print("\n📊 Initializing Support Vector Machine evaluation pipeline...")
                
                # Scale components based on your training directory baseline distributions
                eval_scaler = StandardScaler()
                X_train_sc = eval_scaler.fit_transform(X_train)
                X_eval_sc = eval_scaler.transform(X_eval)
                
                # Run background accuracy modeling
                clf = SVC(kernel="linear", C=1.0, random_state=42)
                clf.fit(X_train_sc, y_train)
                y_pred = clf.predict(X_eval_sc)
                
                final_acc = accuracy_score(y_eval, y_pred)
                final_f1 = f1_score(y_eval, y_pred, average="macro")
