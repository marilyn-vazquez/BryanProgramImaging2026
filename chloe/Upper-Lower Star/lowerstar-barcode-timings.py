...
"""
Timed Lower-Star Barcode Persistence Homology and Machine Learning Experiment
@author: Chloe
"""
...
def build_lower_star_filtration(loaded_image):
    """Convert to float, scale to standard 0-255 domain, return float64 array."""
...
def compute_lower_star_ph(filtration_image):
    """Compute lower-star persistent homology using available Cripser API."""
...
def build_and_time_lower_star_dataset(
    ...
    """Recompute, time, and process all image pipeline stages."""
    ...
    for path in image_paths:
        ...
        # Stage 1: Filtration
        t0 = perf_counter()
        filt_img = build_lower_star_filtration(loaded_img)
        t1 = perf_counter()

        # Stage 2: Persistent Homology
        ph_diag = compute_lower_star_ph(filt_img)
        t2 = perf_counter()
...
def run_experiment(image_paths, output_root):
    ...
    df_data, df_times = build_and_time_lower_star_dataset(
