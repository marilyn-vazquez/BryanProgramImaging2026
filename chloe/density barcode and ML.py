# -*- coding: utf-8 -*-
"""
Created on Tue Jul 14 09:53:02 2026
...
# =====================================================================
# 1. BARCODE VECTORIZATION (FROM RAW DENSITY DIAGRAMS)
# =====================================================================

def vectorize_density_diagrams(diagram_paths):
    """
    Loads raw density persistence diagrams (.npy files) and summarizes 
    ...
    for path in diagram_paths:
        ...
        # Load raw [dim, birth, death] array
        ph = np.load(path)
...
if __name__ == '__main__':
    ...
    # Folder containing your density .npy diagram checkpoints
    PROCESSED_DIR = Path(r"C:\Users\chloe\OneDrive - Simpson College\IMAGES2.0\All Images\preprocessed_images")
   
    # Gather saved raw density diagram arrays
    diagram_paths = sorted(list(PROCESSED_DIR.glob('*_density_diagram.npy')))
   
    if not diagram_paths:
        raise FileNotFoundError(
            f"Could not find any density diagrams (*_density_diagram.npy) in {PROCESSED_DIR}.\n"
            f"Please run your density extraction script first."
        )
    ...
    X_topological_features, y_experimental_classes = vectorize_density_diagrams(diagram_paths)
    ...
    print("\n=== Running Support Vector Machines & Neural Network (Density) ===")
