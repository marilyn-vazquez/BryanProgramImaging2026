# -*- coding: utf-8 -*-
"""
Closing Filtration Persistence Binning Timing Experiment
...
Timing notes:
    ...
    - The filtration timer includes image conversion, thresholding,
      and construction of the complete closing-filtration image.
...
MORPH_TYPE = "closing"
FILTRATION_NAME = "Closing"
VECTORIZATION_METHOD = "Persistence_Binning"
PREPROCESSING_VERSION = "V2"
EXPERIMENT_FOLDER_NAME = "Closing_Preprocessed_V2"
...
def warm_up_morphology_functions(kernel_list):
    """
    Compile the Numba morphology functions before image timing begins.

    Closing uses both dilation and erosion, so both functions are
    called once on a small float64 image using a real kernel.
    """
    warmup_image = np.zeros((5, 5), dtype=np.float64)
    warmup_image[2, 2] = 1.0
...
def build_closing_filtration(
    loaded_image,
    kernel_list,
    threshold=0.5,
):
    """
    Convert the loaded image to binary and build its closing filtration.

    This complete function is timed as the filtration stage.

    Returns:
        filtration_image: accumulated closing-filtration image
...
def build_and_time_closing_dataset(
    image_paths,
    ph_output_dir,
    ...
    """
    Recompute and time all Closing image-processing stages.

    Timed independently for every image:
        1. Thresholding and closing-filtration construction
    ...
    print("Numba morphology functions warmed up successfully.")
    ...
    for image_number, image_path in enumerate(image_paths, start=1):
        ...
        # ---------------------------------------------------------
        # TIME CLOSING FILTRATION CONSTRUCTION
        # ---------------------------------------------------------
        ...
        ph_closing = compute_morphology_ph(
            filtration_image,
            maxdim=1,
        )
        ...
        persistence_0 = ph_closing[
            ph_closing[:, 0] == 0
        ][:, 1:3]
        persistence_1 = ph_closing[
            ph_closing[:, 0] == 1
        ][:, 1:3]

        # Saving occurs after the PH timer stops.
        ph_filename = f"{image_path.stem}_closing_ph.npz"
...
if __name__ == "__main__":
    ...
    print("\n============================================")
    print("CLOSING TIMING EXPERIMENT")
    print("============================================")
    ...
    (
        ...
        manifest_df,
        image_timings_df,
    ) = build_and_time_closing_dataset(
