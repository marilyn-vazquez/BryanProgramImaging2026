# -*- coding: utf-8 -*-
"""
Dilation Filtration Persistence Binning Timing Experiment
...
Timing notes:
    ...
    - The filtration timer includes image conversion, thresholding,
      and construction of the complete dilation-filtration image.
...
def warm_up_morphology_functions(kernel_list):
    """
    Compile the Numba dilation function before image timing begins.

    The dilation function is called once on a small float64 image
...
def build_dilation_filtration(
    loaded_image,
    kernel_list,
    threshold=0.5,
):
    """
    Convert the loaded image to binary and build its dilation filtration.

    This complete function is timed as the filtration stage.

    Returns:
        filtration_image: accumulated dilation-filtration image
...
def build_and_time_dilation_dataset(
    image_paths,
    ph_output_dir,
    ...
    """
    Recompute and time all Dilation image-processing stages.

    Timed independently for every image:
        1. Thresholding and dilation-filtration construction
    ...
    print("Numba dilation function warmed up successfully.")
    ...
    for image_number, image_path in enumerate(image_paths, start=1):
        ...
        # ---------------------------------------------------------
        # TIME DILATION FILTRATION CONSTRUCTION
        # ---------------------------------------------------------
        ...
        ph_dilation = compute_morphology_ph(
            filtration_image,
            maxdim=1,
        )
        ...
        persistence_0 = ph_dilation[
            ph_dilation[:, 0] == 0
        ][:, 1:3]
        persistence_1 = ph_dilation[
            ph_dilation[:, 0] == 1
        ][:, 1:3]

        # Saving occurs after the PH timer stops.
        ph_filename = f"{image_path.stem}_dilation_ph.npz"
...
if __name__ == "__main__":
    ...
    print("\n============================================")
    print("DILATION TIMING EXPERIMENT")
    print("============================================")
    ...
    (
        ...
        manifest_df,
        image_timings_df,
    ) = build_and_time_dilation_dataset(
